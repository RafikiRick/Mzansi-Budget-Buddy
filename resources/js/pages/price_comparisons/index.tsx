...
const PriceEntry = ({ entry }) => {
    ...
    const priceColor = entry.is_best_deal ? 'text-green-600 dark:text-green-400' : 'text-neutral-900 dark:text-neutral-100';
        ...
        >
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-400">{entry.store} | {new Date(entry.updated_at).toLocaleDateString()}</p>
                </div>
            </div>
...
export default function PriceComparisonIndex() {
    ...
    const filteredAndSortedResults = useMemo(() => {
        ...
        let results = priceComparisons.filter(item =>
                    ...
                    <div className="flex-1 overflow-y-auto divide-y divide-neutral-200/60 dark:divide-neutral-700/60">
                        ...
                        {filteredAndSortedResults.length === 0 && (
                            <div className="p-4 text-center text-neutral-500 dark:text-neutral-400">
                                {priceComparisons.length === 0 ? 'No price data available yet.' : 'No price results found. Please adjust your search.'}
                            </div>
                        )}
                    </div>
                </div>
